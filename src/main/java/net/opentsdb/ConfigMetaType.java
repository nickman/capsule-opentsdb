...
import net.opentsdb.ConfigReader.ConfigurationItem;
...
public enum ConfigMetaType implements ArgValueValidator {
	...
	NULL("No validator", new ArgValueValidator(){
		@Override
		public void validate(final ConfigurationItem citem) {/* No Op */}
	}),
	/** An HBase table name */
	...
	/**
	 * {@inheritDoc}
	 * @see net.opentsdb.ArgValueValidator#validate(net.opentsdb.ConfigReader.ConfigurationItem)
	 */
	@Override
	...
	public static class ReadWriteModeValidator implements ArgValueValidator {
		...
		public void validate(final ConfigurationItem citem) {			
			try {
				final String _mode = citem.getValueStr();
				if(!MODES.contains(_mode)) throw new Exception();
			} catch (Exception ex) {
				throw new IllegalArgumentException("Invalid ReadWrite Mode [" + citem.getValueStr() + "] for " + citem.getKey());
			}
		}
	}
	...
	public static class ClassListValidator implements ArgValueValidator {
		...
		@Override
		public void validate(final ConfigurationItem citem) {			
			final String val = citem.getValueStr();
			...
			for(String className: classNames) {
				...
				try {
					...
					throw new IllegalArgumentException("Failed to load class [" + className + "] defined in configuration item [" + citem.getKey() + "]", ex);
				}
			}
		}
	}
	...
	public static class IntegerValidator implements ArgValueValidator {
		...
		public void validate(final ConfigurationItem citem) {			
			try {
				int i = Integer.parseInt(citem.getValueStr());
				if(i < min) throw EX; 
			} catch (Exception ex) {
				throw new IllegalArgumentException("Invalid Integer value [" + citem.getValueStr() + "] for " + citem.getKey());
			}
		}
	}
	...
	public static class BooleanValidator implements ArgValueValidator {
		...
		@Override
		public void validate(final ConfigurationItem citem) {
			if(!"true".equalsIgnoreCase(citem.getValueStr()) && !"false".equalsIgnoreCase(citem.getValueStr())) {
				throw new IllegalArgumentException("Invalid Boolean value [" + citem.getValueStr() + "] for " + citem.getKey());				
			}
		}
	}
	...
	public static class URLOrFileValidator implements ArgValueValidator {
		...
		public void validate(final ConfigurationItem citem) {
			try {
				new URL(citem.getValueStr());
			} catch (Exception ex) {
				File f = new File(citem.getValueStr());
				if(!f.exists() || !f.isFile() || !f.canRead()) {
					throw new IllegalArgumentException("Invalid URL or File [" + citem.getValueStr() + "]", ex);
				}
			}
		}
	}
	...
	public static class StringValidator implements ArgValueValidator {
		...
		@Override
		public void validate(final ConfigurationItem citem) {
			if(citem.getValueStr()==null || citem.getValueStr().trim().isEmpty()) {
				throw new IllegalArgumentException("Null or empty " + name + " value for " + citem.getKey());
			}
		}
	}
	...
	public static class BootLoadableClass extends StringValidator {
		...
		/**
		 * {@inheritDoc}
		 * @see net.opentsdb.ConfigMetaType.StringValidator#validate(net.opentsdb.ConfigReader.ConfigurationItem)
		 */
		@Override
		public void validate(final ConfigurationItem citem) {
			super.validate(citem);
			try {
				Class.forName(citem.getValueStr().trim(), true, ConfigMetaType.class.getClassLoader());
			} catch (Exception ex) {
				throw new IllegalArgumentException("Failed to load boot time class [" + citem.getValueStr() + "] for " + citem.getKey());
			}
		}
	}
	...
	public static class ClasspathConfiguredClassList implements ArgValueValidator {
		...
		@Override
		public void validate(final ConfigurationItem citem) {
			Set<String> classNames = new LinkedHashSet<String>(Arrays.asList(COMMA_SPLITTER.split(citem.getValueStr())));
			...
			try {
				...
				throw new IllegalArgumentException("Failed to load boot time class [" + className + "] for " + citem.getKey());
			}
		}
	}
	...
	public static class DirFileSystemValidator implements ArgValueValidator {
		...
		@Override
		public void validate(final ConfigurationItem citem) {
			final String name = citem.getValueStr();
			if(name==null || name.trim().isEmpty()) return;
			final File f = new File(citem.getValueStr()).getAbsoluteFile();
	...
	public static class FileSystemValidator implements ArgValueValidator {
		...
		@Override
		public void validate(final ConfigurationItem citem) {
			String value = citem.getValueStr();
			File target = new File(value);
			if(mustExist) {
				if(!target.exists()) throw new IllegalArgumentException((dir ? "Directory [" : " File [") + value + "] does not exist for " + citem.getKey());
				if(dir) {
					if(!target.isDirectory()) throw new IllegalArgumentException(("[") + value + "] is a file not a directory for " + citem.getKey());
				} else {
					if(!target.isFile()) throw new IllegalArgumentException(("[") + value + "] is a directory not a file for " + citem.getKey());
				}
			} else {
				if(dir) {
					if(!target.exists()) {
						if(!target.mkdirs()) throw new IllegalArgumentException(("Could not create directory [") + value + "] for " + citem.getKey());
					} else {
						if(!target.isDirectory()) throw new IllegalArgumentException(("[") + value + "] is a file not a directory for " + citem.getKey());
					}
				} else {
					if(!target.getParentFile().exists()) {
						if(!target.getParentFile().mkdirs()) throw new IllegalArgumentException(("Could not create parent directory for file [") + value + "] for " + citem.getKey());
					}
					
					if(!target.exists()) {
						try {						
							if(!target.createNewFile()) throw new IllegalArgumentException(("Could not create file [") + value + "] for " + citem.getKey());
						} catch (IOException e) {
							throw new IllegalArgumentException(("Could not create file [") + value + "] for " + citem.getKey());
						} finally {
							target.delete();
						}
					}
...
//		private static final TimeZone GMT = TimeZone.getTimeZone("GMT");
		...
		@Override
		public void validate(final ConfigurationItem citem) {
			final String tz = citem.getValueStr();
	...
	public static class FileListValidator implements ArgValueValidator {
		...
		protected Set<URL> buildURLSet(final ConfigurationItem citem) {
			...
			String[] files = COMMA_SPLITTER.split(citem.getValueStr());
...
//					}
			...
			if(!failed.isEmpty()) {
				...
				throw new IllegalArgumentException("Invalid Files or URLs in File List for " + citem.getKey() + "\n" + b.toString());				
			}
	...
	public static class ClassPathValidator extends FileListValidator {
		...
		/**
		 * {@inheritDoc}
		 * @see net.opentsdb.ConfigMetaType.FileListValidator#validate(net.opentsdb.ConfigReader.ConfigurationItem)
		 */
		@Override
		public void validate(final ConfigurationItem citem) {
			super.validate(citem);
			...
			try {
				...
				throw new IllegalArgumentException("Failed to create class loader for [" + citem.getKey() + "]", ex);	
			}
